...
Future<void> _markTaskIncomplete(Map task) async {
  ...
  if (success) {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text('Failed to update task flutter')),
    );
  }
}
  ...
  return Scaffold(
    ...
    bottomNavigationBar: BottomNavigationBar(
      ...
      onTap: (i) async {
        setState(() {
          isDailyMode = false;
          isAllRecordsMode = false; // 🔥 important
          selectedIndex = i;
          isCompletedView = false;  // ✅ RESET HEADER HERE
          tasks.clear();
          offset = 0;
          hasMore = true;

          if (i == 0) currentFilter = 'TODAY'; // will fetch both
          if (i == 1) currentFilter = 'UPCOMING';
          if (i == 2) currentFilter = 'ALL';
       
        });

        await _loadTasks(filter: currentFilter);
      },
